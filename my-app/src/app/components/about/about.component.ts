...
export class AboutComponent implements OnInit {
  ...
  constructor(private router: Router) {
    ...
    const state = navigation.extras.state as { medID };

    for (let i = 0; i < this.medList.length; i++) {
      if (this.medList[i].idMed === state.medID) {
        for (let j = 0; j < this.medList[state.medID].doctors.length; j++) {
          this.doctorsToShow.push(this.medList[state.medID].doctors[j])
        }
      }
    }
  }
